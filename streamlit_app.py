...
def get_route(coord1, coord2):
    ...
    try:
        r = requests.get(url, params=params, headers=headers, timeout=30)  # Augmentez le timeout à 30 secondes
        r.raise_for_status()  # Lève une exception si la requête échoue
        data = r.json()
        distance = round(data["features"][0]["properties"]["segments"][0]["distance"] / 1000, 0)
        duration = data["features"][0]["properties"]["segments"][0]["duration"] / 60
        coords = data["features"][0]["geometry"]["coordinates"]
        return distance, coords, duration
...
def find_itineraire(df, start, end, max_steps=10):
    ...
    return None, None, None
#----------------------------------
# 👇 ICI TU AJOUTES TON CHATBOT
#---------------------------------
def chatbot_response(question, df=None):
    q = question.lower()

    if "train" in q:
        return "🚆 Le train est généralement le transport le moins émetteur de CO2."

    if "avion" in q:
        return "✈️ L’avion est le mode de transport le plus émetteur de CO2."

    if "voiture" in q:
        return "🚗 La voiture dépend fortement du carburant et du nombre de passagers."

    if "co2" in q:
        return "🌍 Le CO2 varie selon le mode de transport et la distance parcourue."

    if "meilleur" in q:
        return "💡 En général : Train < Voiture < Avion en termes d’émissions CO2."

    return "🤖 Je peux t'aider sur les transports, émissions CO2, ou comparer les trajets."
# -------------------------
...
# SIDEBAR CHATBOT
# -------------------------
st.sidebar.title("🤖 Assistant CO2")

user_question = st.sidebar.text_input("Pose ta question")
...
if depart != arrivee and st.session_state.calcul:
    ...
    if distance_route:
        ...
        # CARTES
        # -------------------------
        st.subheader("📊 Comparaison-Total CO2")
